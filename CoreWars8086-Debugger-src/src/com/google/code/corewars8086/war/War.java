...
import com.google.code.corewars8086.memory.RealModeMemoryImpl;
		...
		for (int i = 0; i < m_numWarriors; ++i) {
			...
			if (warrior.isAlive()) {
				try {
					...
					m_warListener.onWarriorDeath(warrior, "memory exception");
					warrior.kill();
					--m_numWarriorsAlive;
				}
			}
		...
		for (int i = 0; i < warriors.size(); ++i) {
			...
			// notify listener
                        /*Thread  t=new Thread(){
                            public void run()
                            {
                                JOptionPane.showMessageDialog(null,m_warriors[m_numWarriors-1].m_state,m_warriors[m_numWarriors-1].m_name, JOptionPane.PLAIN_MESSAGE);
                            }
                        };
                        t.start();*/
			m_warListener.onWarriorBirth(m_warriors[m_numWarriors-1]);
		}
	...
	private short getLoadOffset(int warriorSize) throws Exception {
		...
		while ((!found) && (numTries < MAX_LOADING_TRIES)) {
			...
			for (int i = 0; i < m_numWarriors; ++i) {
				...
				if ((loadAddress+warriorSize >= otherStart) &&
						(loadAddress < otherEnd)) {
					found = false;
				}
			}
		}
